...
class Host(object):
    ...
    def run(self, argvs, quiet=False):
        ...
        """
        if not argvs or not isinstance(argvs, (list, str)):
        ...
        LOG.debug('    [%s@%s]$ %s', self.user, self.hostname, str(argvs))
    ...
    def exec_mongo_command(self,
        ...
        # connection_string can contain ampersands, escape them for shell.
    ...
    def kill_remote_procs(self,
        ...
        :param int|str signal_number: The signal to send, defaults to SIGKILL(9). It should be a
                                      valid signal.
