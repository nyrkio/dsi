...
class ConfigDictTestCase(unittest.TestCase):
    ...
    def test_iterators(self):
        ...
        print(actual_values)
        expected_values = [
            'https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-amazon-3.4.6.tgz', '53.1.1.7',
            '10.2.1.7'
        ]
        expected_dict = {
            'setParameter': {
    ...
    def test_lookup_path_ex(self):
        ...
        conf = self.conf['infrastructure_provisioning']['out']
        self.assertRaisesRegex(KeyError, "ConfigDict: Key not found: MONGOD in path \\['MONGOD'\\]",
